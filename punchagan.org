...
#+END_SRC
**** Post from Org-mode to Zulip
#+BEGIN_SRC emacs-lisp
  (require 'ox-gfm)

  (defun pc/zulip-send-message (realm stream topic message)
    "Send a message to a given realm, stream & topic"
    (call-process "zulip-send" nil t t
                  "--site" realm
                  "-s" stream
                  "-S" topic
                  "--config-file" (expand-file-name (format "%s.zrc" realm) "~/.zuliprc")
                  "-m" message
                  "-v"))

  (defun pc/zulip-get-subtree-content ()
    (let ((org-export-with-toc nil)
          (org-export-show-temporary-export-buffer nil)
          (buffer (org-gfm-export-as-markdown nil t t))
          (char-re ))
      (with-current-buffer buffer
        (goto-char (point-min))
  ...
  (defun pc/zulip-send-org-subtree ()
    (interactive)
    (let* ((realm (cdr (assoc "ZULIP_REALM" (org-entry-properties (point) "ZULIP_REALM"))))
           ...
           (message (pc/zulip-get-subtree-content)))
      (pc/zulip-send-message realm stream topic message)))
